...
Функция ПодготовитьПостроениеГрафа(ДеревоФункций, ИДСтроки, GML_Флаг) Экспорт
	GML = GML_Флаг;
	...
	Если GML И ТипОтчета = 1 И ТипОтчетаПоСекциям = 1 Тогда
		Возврат ПодготовитьПостроениеГрафаВызовыМеждуСекциямиGML(ДеревоФункций, Структура_GML);
	ИначеЕсли ТипОтчета = 1 И ТипОтчетаПоСекциям = 1 Тогда
	...
	Если  GML Тогда
		...
		Результат = ЗавершитьПостроениеГрафаGML(ИдентификаторыФункций, Структура_GML, ТекстГрафа);		
	Иначе	
...
Функция ЗавершитьПостроениеГрафаGML(Знач ИдентификаторыФункций, Знач Структура_GML, ТекстГрафа)
	Перем Результат;
	// Завершим вывод текста графа
...
Функция ПодготовитьПостроениеГрафаВызовыМеждуСекциямиGML(ДеревоФункций, Структура_GML)
	Перем НомерКластера;
	Перем EdgeS;
    Перем Node_ID;
    Перем NodeS;
...
Функция ДобавитьВТаблицуФункции(ИмяФункции, СтруктураРазбора, Директива, GML=Ложь)
	
	СтруктураРазбора.НомерФункции = СтруктураРазбора.НомерФункции + 1;
	новСтр = СтруктураРазбора.Функции.Добавить();
	Если GML Тогда
	новСтр.Идентификатор = "" + Формат(СтруктураРазбора.НомерФункции, "ЧЦ=3; ЧВН=");   
	Иначе
...
Функция ПервыйПроход(ТекстМодуля, ДляУправляемойФормы, СтруктураРазбора)
	...
	Для Сч = 1 По ТекстМодуля.КоличествоСтрок() Цикл
		...
		Если ЯвляетсяФункциейИлиПроцедурой(ПервоеСлово, ЭтоФункция) Тогда
			...
			новСтр = ДобавитьВТаблицуФункции(ИмяФункции, СтруктураРазбора, Директива, GML);
			...
			Если ДляУправляемойФормы И Не Директива82Серверная(СтрФункции.Директива) Тогда
				...
				Для Каждого Элемент Из СтруктураРазбора.ПриводящиеКВызовуНаСервер Цикл
					...
					Если Вызовов <> 0 Тогда
						...
						Если ИДФункции = Неопределено Тогда
							новСтр = ДобавитьВТаблицуФункции(Элемент.Значение, СтруктураРазбора, Директива82ДляСерверныхФункций(), GML);
				...
				Для Каждого Элемент Из СтруктураРазбора.ОбщиеМодули Цикл
					...
					Если Вызовов <> 0 Тогда
						...
						Если ИДФункции = Неопределено Тогда
							новСтр = ДобавитьВТаблицуФункции(Элемент.Значение, СтруктураРазбора, Директива82ДляСерверныхОбщихМодулей(), GML);
							ИДФункции = новСтр.Идентификатор;
